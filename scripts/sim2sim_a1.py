...
default_joint_angles = {
            ...
            'FL_thigh_joint': 0.8,   # [rad] LF_HFE 
            'RL_thigh_joint': 0.8,   # [rad] LH_HFE
            'FR_thigh_joint': 0.8,   # [rad] RF_HFE
            'RR_thigh_joint': 0.8,   # [rad] RH_HFE

            'FL_calf_joint': -1.5,   # [rad] LF_KFE
            'RL_calf_joint': -1.5,   # [rad] LH_KFE
            'FR_calf_joint': -1.5,   # [rad] RF_KFE
            'RR_calf_joint': -1.5,   # [rad] RH_KFE
        }
...
def get_obs(data, model):
    ...
    obs_delay = obs_buffer.get_delayed_value(0.00)
...
class Sim2simCfg():
    ...
    class sim_config:
        mujoco_model_path = f'legged_lab/assets/a1/scene.xml'
        sim_duration = 60.0
        dt = 0.004
        decimation = (0.02 / dt)
        num_actions = 12
    ...
    class robot_config:
        ...
        kps = np.array([20.0]*12, dtype=np.double)
        kds = np.array([0.5]*12, dtype=np.double)
        tau_limit = 120. * np.ones(12, dtype=np.double)
        # kps[[2, 5, 8, 11]] = 60.0
...
def run_mujoco(policy, estimator, lidar_encoder, cfg: Sim2simCfg):
    ...
    observations_list = ["ang_vel_body", "gravity_vec", "commands", "dof_pos", "dof_vel", "actions"]
    ...
    obs_history = CircularBuffer(max_len=11, batch_size=1, device=device)
            ...
            if count_lowlevel % cfg.sim_config.decimation == 0:
                ...
                for obs_name in observations_list:
                    ...
                    if obs_name == "ang_vel_body":
                        ...
                        obs_list.extend([-joy_data.Axis[1] * 4.0 + cmd.vx * 2.0,
                                        -joy_data.Axis[0]  * 2.0 + cmd.vy * 2.0,
                                        -joy_data.Axis[3] * 0.25 + cmd.dyaw * 0.25])
                ...
                command[0] = -joy_data.Axis[1] * 4.0 + cmd.vx * 2.0
                command[1] = -joy_data.Axis[0]  * 2.0 + cmd.vy * 2.0
                command[2] = -joy_data.Axis[3] * 0.25 + cmd.dyaw * 0.25
    ...
    while (done != True):
        for event in pygame.event.get():
            if event.type == pygame.JOYAXISMOTION:
                ...
                joy_data.Axis[event.axis] = event.value
                if abs(joy_data.Axis[event.axis]) < 0.1:
                    joy_data.Axis[event.axis] = 0
                # print(joy_data.Axis, event.axis)
            elif event.type == pygame.JOYBUTTONDOWN:
                # print("Button {} pressed".format(event.button))
                joy_data.button_last = joy_data.button
                joy_data.button = event.button
        time.sleep(0.002)

# 这里是main函数
if __name__ == '__main__':
    # 注意这里加载的是policy，不是model
    path = f'logs/a1_roa/2025-03-22_11-48-06/exported/base_jit.pt'
    # path = f'logs/ac2_flat/2025-03-22_11-48-06/exported/policy_1.pt'
