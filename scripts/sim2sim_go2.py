...
default_joint_angles = {
            ...
            'RL_thigh_joint': 1.0,   # [rad] LH_HFE
            'FR_thigh_joint': 0.8,   # [rad] RF_HFE
            'RR_thigh_joint': 1.0,   # [rad] RH_HFE
...
class Sim2simCfg():
    ...
    class sim_config:
        mujoco_model_path = f'assets/go2/scene_terrain.xml'
        sim_duration = 60.0
        dt = 0.005
        decimation = (0.02 / dt)
        num_actions = 12
    ...
    class robot_config:
        ...
        torque_limit = 30. * np.ones(12, dtype=np.double)
...
def run_mujoco(policy, estimator, lidar_encoder, cfg: Sim2simCfg):
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
        ...
        while viewer.is_running():
            ...
            plot(q, "q")
            plot(dq, "dq")
            phase = (count_lowlevel * cfg.sim_config.dt) % 0.5 / 0.5
            ...
            tau = np.clip(tau, -cfg.robot_config.torque_limit, cfg.robot_config.torque_limit)
            global disable_mode
            if (abs(tau) > cfg.robot_config.torque_limit).any():
                disable_mode = True
...
if __name__ == '__main__':
    #TODO Policy path
    path = f'pre-trian_policy/amp_vae_v4.pt'
    if path == f'pre-trian_policy/amp_vae_v4.pt':
        policy_class = "go2_vae"
    else:
        policy_class = "go2_amp"
