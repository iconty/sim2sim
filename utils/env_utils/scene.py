...
class SceneCfg(InteractiveSceneCfg):
    ...
    def __init__(
        ...
        if config.lidar_scanner.enable_lidar_scan or config.lidar_scanner.debug_mode:
            self.lidar_scanner = RayCasterCfg(
                prim_path="{ENV_REGEX_NS}/Robot/" + config.lidar_scanner.prim_body_name,
                offset=RayCasterCfg.OffsetCfg(pos=(0.0, 0.0, 0.0)),
                attach_yaw_only=False, # lidar must be rotated with the robot
                pattern_cfg=patterns.LidarPatternCfg(
                    channels=config.lidar_scanner.channels,
                    vertical_fov_range=config.lidar_scanner.vertical_fov_range,
                    horizontal_fov_range=config.lidar_scanner.horizontal_fov_range,
                    horizontal_res=config.lidar_scanner.horizontal_res
                ),
                max_distance=config.lidar_scanner.max_distance,
                debug_vis=config.lidar_scanner.debug_vis,
                mesh_prim_paths=["/World/ground"],
                update_period=step_dt
            )
